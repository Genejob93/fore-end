...
```js
1. 生成 package.json 文件.    -- 即: 让文件夹成为一个合法包
2. 安装 webpack (全局和局部都要安装)
...
## 1.5 在package.json 中配置npm命令

> 在package.json中 配置 npm 命令, 目的是把 平时执行 `webpack` 这个命令替换为 `npm run build` 这个命令.  配置完这个命令后, 再打包的时候就可以使用 `npm run build` 这个命令. 
...
## 1.6 使用 loader 解析less 文件 (使用 less-loader)
